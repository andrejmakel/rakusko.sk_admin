                ...
                <tbody>
                    @foreach($posts as $key => $post)
                        <tr data-entry-id="{{ $post->id }}">
                            ...
                            <td>
                                {{ $post->originTransPosts[0]->order ?? '' }}
                            </td>
                            <td>
                                <a href="{{ route('admin.posts.edit', $post->id) }}">
                                    {{ $post->originTransPosts[0]->title ?? '' }}
                                </a>
                            </td>
                            <td>
                            ...
                            <td>
                                @foreach($post->activities as $key => $item)
                                    <span class="badge badge-info">{{ $item->title_de }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ $post->created_at ?? '' }}
...
{{--                                 @can('post_create')
                                <a class="btn btn-xs btn-success" href="posts/clone/{{$post->id}}">
                                    Clone
                                </a>
                                @endcan --}}
