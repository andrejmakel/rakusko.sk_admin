...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.mall.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.malls.update", [$mall->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            {{-- Mall --}}
              ...
              <div class="d-flex justify-content-start my-3">
                <a href="/admin/malls" class="btn btn-warning m-1" onclick="return confirm('{{ trans('global.areYouSure') }}');">
                  Back
                </a>
              ...
              <div class="row">
                <div class="form-group col-6">
                  ...
                  <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $mall->title) }}" required>
                  @if($errors->has('title'))
                ...
                <div class="form-group col-6">
                  ...
                  <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $mall->slug) }}" required>
                  @if($errors->has('slug'))
                ...
                <div class="form-group col-8">
                  ...
                  <input class="form-control {{ $errors->has('map_embed') ? 'is-invalid' : '' }}" type="text" name="map_embed" id="map_embed" value="{{ old('map_embed', $mall->map_embed) }}" required>
                ...
                <div class="form-group col-2">
                  ...
                  <input class="form-control {{ $errors->has('zip') ? 'is-invalid' : '' }}" type="number" name="zip" id="zip" value="{{ old('zip', $mall->zip) }}" step="1">
                  @if($errors->has('zip'))
                ...
                <div class="form-group col-2">
                  ...
                  <input class="form-control date {{ $errors->has('update') ? 'is-invalid' : '' }}" type="text" name="update" id="update" value="{{ old('update', $mall->update) }}">
        ...
        @foreach ($mall->originTransMalls as $key => $transMall)
          <div id="form-{{$transMall->lang->lang}}" class="d-{{ $key === 0 ? 'block' : 'none' }} form">

              <div class="form-group d-none">
                <label class="required" for="lang_id">{{ trans('cruds.transMall.fields.lang') }}</label>
                <select class="form-control select2 {{ $errors->has('lang') ? 'is-invalid' : '' }}" name="transData[{{$transMall->lang->id}}]['lang_id']" id="lang_id" required>
                    @foreach($langs as $id => $entry)
                        <option value="{{$transMall->lang->id}}" selected>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('lang'))
            ...
            <div class="row">
              ...
              <div class="form-group col-2">
                ...
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" value="{{$transMall->order}}" name="transData[{{$transMall->lang->id}}]['order']" id="order" value="{{ old('order', '') }}" step="1">
                @if($errors->has('order'))
              ...
              <div class="form-group col-5">
                  ...
                  <input class="form-control {{ $errors->has('subtitle') ? 'is-invalid' : '' }}" type="text" value="{{$transMall->subtitle}}" name="transData[{{$transMall->lang->id}}]['subtitle']" id="subtitle" value="{{ old('subtitle', '') }}">
              ...
              <div class="form-group col-5">
                  ...
                  <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" value="{{$transMall->link}}" name="transData[{{$transMall->lang->id}}]['link']" id="link" value="{{ old('link', '') }}">
                  @if($errors->has('link'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('text') ? 'is-invalid' : '' }}" name="transData[{{$transMall->lang->id}}]['text']" id="text">{{$transMall->text}}</textarea>
                @if($errors->has('text'))
