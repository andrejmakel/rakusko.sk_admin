    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shops.update", [$shop->id]) }}" enctype="multipart/form-data">
            ...
            <div class="d-flex justify-content-start my-3">
              <a href="/admin/shops" class="btn btn-warning m-1" onclick="return confirm('{{ trans('global.areYouSure') }}');">
                Back
              </a>
              ...
              <div class="col-8">
                <div class="row">
                  ...
                  <div class="form-group col-3">
                    ...
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $shop->title) }}" required>
                    @if($errors->has('title'))
                  ...
                  <div class="form-group col-3">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $shop->slug) }}" required>
                    @if($errors->has('slug'))
                  ...
                  <div class="form-group col-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                        @foreach($categories as $id => $category)
                            <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $shop->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
                  ...
                  <div class="form-group col-3">
                    ...
                    <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="text" name="discount" id="discount" value="{{ old('discount', $shop->discount) }}">
                  ...
                  <div class="form-group col-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('mall') ? 'is-invalid' : '' }}" name="mall_id" id="mall_id" required>
                        @foreach($malls as $id => $entry)
                            <option value="{{ $id }}" {{ (old('mall_id') ? old('mall_id') : $shop->mall->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('mall'))
                  ...
                  <div class="form-group col-3">
                    ...
                    <input class="form-control {{ $errors->has('zip') ? 'is-invalid' : '' }}" type="number" name="zip" id="zip" value="{{ old('zip', $shop->zip) }}" step="1">
                    @if($errors->has('zip'))
                  ...
                  <div class="form-group col-3">
                    ...
                    <input class="form-control date {{ $errors->has('update') ? 'is-invalid' : '' }}" type="text" name="update" id="update" value="{{ old('update', $shop->update) }}">
            ...
            @foreach ($shop->originTransShops as $key => $transShop)

              <div id="form-{{$transShop->lang->lang}}" class="d-{{ $key === 0 ? 'block' : 'none' }} form">

                <div class="form-group d-none">
                  <label class="required" for="lang_id">{{ trans('cruds.transPlace.fields.lang') }}</label>
                  <select class="form-control select2 {{ $errors->has('lang') ? 'is-invalid' : '' }}" name="transData[{{$transShop->lang->id}}]['lang_id']" id="lang_id" required>
                      @foreach($langs as $id => $entry)
                          <option value="{{$transShop->lang->id}}" selected>{{ $entry }}</option>
                      @endforeach
                  </select>
                  @if($errors->has('lang'))
                ...
                <div class="row">
                  ...
                  <div class="form-group col-3">
                    ...
                    <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" value="{{$transShop->order}}" name="transData[{{$transShop->lang->id}}]['order']" id="order" value="{{ old('order', '') }}" step="1">
                    @if($errors->has('order'))
                  ...
                  <div class="form-group col-9">
                    ...
                    <input class="form-control {{ $errors->has('subtitle') ? 'is-invalid' : '' }}" type="text" value="{{$transShop->subtitle}}" name="transData[{{$transShop->lang->id}}]['subtitle']" id="subtitle" value="{{ old('subtitle', '') }}">
                ...
                <div class="row">
                  ...
                  <div class="form-group col-6">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('text') ? 'is-invalid' : '' }}" name="transData[{{$transShop->lang->id}}]['text']" id="text">{{$transShop->text}}</textarea>
                    @if($errors->has('text'))
                  ...
                  <div class="form-group col-6">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('opening_note') ? 'is-invalid' : '' }}" name="transData[{{$transShop->lang->id}}]['opening_note']" id="opening_note">{{$transShop->opening_note}}</textarea>
