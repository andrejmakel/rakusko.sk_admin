...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.transInfo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trans-infos.update", [$transInfo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
...
{{--             <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lang') ? 'is-invalid' : '' }}" name="lang_id" id="lang_id" required>
                    @foreach($langs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lang_id') ? old('lang_id') : $transInfo->lang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('lang'))
            ...
            <div class="form-group">
                <label class="required" for="origin_id">{{ trans('cruds.transInfo.fields.origin') }}</label>
                <select class="form-control select2 {{ $errors->has('origin') ? 'is-invalid' : '' }}" name="origin_id" id="origin_id" required>
                    @foreach($origins as $id => $entry)
                        <option value="{{ $id }}" {{ (old('origin_id') ? old('origin_id') : $transInfo->origin->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('origin'))
                    <span class="text-danger">{{ $errors->first('origin') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.transInfo.fields.origin_helper') }}</span>
            </div> --}}

            <div class="form-group">
                <label for="order">{{ trans('cruds.transInfo.fields.order') }}</label>
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $transInfo->order) }}" step="1">
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $transInfo->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subtitle') ? 'is-invalid' : '' }}" type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $transInfo->subtitle) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $transInfo->slug) }}">
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('text') ? 'is-invalid' : '' }}" name="text" id="text">{!! old('text', $transInfo->text) !!}</textarea>
                @if($errors->has('text'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $transInfo->link) }}">
                @if($errors->has('link'))
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.trans-infos.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $transInfo->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
