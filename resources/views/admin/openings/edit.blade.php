...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.opening.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.openings.update", [$opening->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="open_hours">{{ trans('cruds.opening.fields.open_hours') }}</label>
                <input class="form-control {{ $errors->has('open_hours') ? 'is-invalid' : '' }}" type="text" name="open_hours" id="open_hours" value="{{ old('open_hours', $opening->open_hours) }}" required>
